...
def compute_start(motion: dict) -> int:
    """Return frame index to start using after jump-based trimming."""
    raw_pos = motion["root_pos"].astype(np.float32)
    return find_stable_start(raw_pos)
...
def main():
    ...
    for pkl_path in pkl_files:
        ...
        trim_note = f"  [trimmed {start}f: jump={start}f]" if start > 0 else ""
    ...
    if not all_motions:
        raise RuntimeError("No motions survived filtering — check JUMP_THRESHOLD_M")
